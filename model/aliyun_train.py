...
def inception_resnet_v2(inputs, num_classes=1001, is_training=True,
  ...
  Args:
    inputs: 输入为一个4维张量 a 4-D tensor of size [batch_size, height, width, 3].
  ...
  with tf.variable_scope(scope, 'InceptionResnetV2', [inputs],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        # todo 设置一个全局池的变量能够在这用来控制池化 例如 resnet_*()
...
def generate_filenamequeue(filequeuelist):
    filename_queue = tf.train.string_input_producer([filequeuelist], num_epochs=50) #num_epochs 整个样本的循环迭代次数
...
def main(_):
    ...
    test_file_path = os.path.join(FLAGS.buckets, "test_train.tfrecords")
    ckpt_path = os.path.join(FLAGS.checkpointDir, "model.ckpt")
    batch_size = 20
    lr = tf.Variable(0.0005, dtype=tf.float32)  # 下降梯度
    x_train = tf.placeholder(tf.float32, [batch_size, 299, 299, 3], name="data-input")
    y_train = tf.placeholder(tf.int64, [batch_size, ], name="label-input")  # 训练集 num_class = 12
    is_training = tf.placeholder(tf.bool, name="is_training")
    step = 0

    logits, endpoint = inception_resnet_v2(x_train, num_classes=12, is_training=is_training)
    # 创建损失函数
    loss = tf.nn.softmax_cross_entropy_with_logits(labels=tf.one_hot(y_train, 12), logits=logits)
    ...
    try:
        images, labels = get_batch(generate_filenamequeue(train_file_path), batch_size)
        test_images, test_labels = get_batch(generate_filenamequeue(test_file_path), batch_size)
        # 初始化
        init_op = tf.group(tf.global_variables_initializer(), tf.local_variables_initializer())
        saver = tf.train.Saver()
        with tf.Session() as sess:
            ckpt = tf.train.get_checkpoint_state(
                'oss://vagetabledata.oss-cn-beijing-internal.aliyuncs.com/model_output')
            if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
                saver.restore(sess, ckpt.model_checkpoint_path)
            else:
                sess.run(init_op)

            summary_writer = tf.summary.FileWriter('oss://vagetabledata.oss-cn-beijing-internal.aliyuncs.com/logs',
                                                   sess.graph)
            ...
            # 队列读取未停止，迭代次数小于10000
            while not coord.should_stop() and step <= 10001:
                ...
                if step >= 1000 and step % 100 == 0:
                    ...
                    if acc0 >= 0.85:
                        ...
                        # sess.save(sess,os.path.join(os.getcwd(), 'inception_resnet_v3_swc.ckpt'),global_step = i)
                        print("Model done")
                        break
                    # 每迭代2000次，降低一次学习率
                    if step % 200 == 0:
